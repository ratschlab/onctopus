...
def get_CNV_list_from_result_file(line_part, phase=-1):
	...
	for i in range(len(cnvs)):
		...
		cnv_list.append(cnv_object.CNV(int(change), int(seg_index), int(chr), int(start), int(end)))
...
def create_some_fixed_files(lineages, fixed_cnv_file=None, fixed_ssm_file=None, 
	...
	if fixed_cnv_file:
		model.create_fixed_file(lineages, cons.CNV, output_file=fixed_cnv_file)
	if fixed_ssm_file:
		model.create_fixed_file(lineages, cons.SSM, output_file=fixed_ssm_file)
	if fixed_z_matrix_file:
		model.create_fixed_file(lineages, cons.Z, output_file=fixed_z_matrix_file)
	if fixed_phi_file:
		model.create_fixed_file(lineages, cons.FREQ, output_file=fixed_phi_file)
	...
	with open(input_SNPs_file, 'r') as input_file:
		with open(output_BAF_file, 'w') as output_file:
			...
			for line_num, line in enumerate(input_file, start = 1):
				...
				# try to compute BAF
				try:
					sample = model.compute_BAF(split_line[mapping_base_to_index[variance]], split_line[6])
...
def SNP_count_to_LogR_new(input_SNPs_file, output_LogR_file, avg_coverage=0, normal=True, test=False,
	...
	if normal:
		my_compute_LogR = model.compute_LogR_normal
	else:
		my_compute_LogR = model.compute_LogR_tumor
	my_compute_LogR_star = model.compute_LogR_star
	...
	my_median = model.median_list(logR)
	...
	with open(input_SNPs_file, 'r') as input_file:
		with open(output_LogR_file, 'w') as output_file:
			...
			for line_num, line in enumerate(input_file, start = 1):
				...
				split_line = map(int, line.rstrip("\n").split("\t"))
				# compute LogR 
				sample = model.compute_LogR(split_line[6], avg_coverage)
